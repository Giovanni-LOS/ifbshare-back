...
class PostController implements IPostController {
    ...
    public getPosts: RequestHandler<Record<string, unknown>, Record<string, unknown>, Record<string, unknown>> = async (_req, res) => {
        const posts = await this.postDAO.findAll();
    ...
    public getPostsById: RequestHandler<HeaderId, Record<string, unknown>, Record<string, unknown>> = async (req, res) => {
        const { id } = req.params;
    ...
    public createPost: RequestHandler<Record<string, unknown>, Record<string, unknown>, CreatePostBody> = async (req, res) => {
        ...
        const post = await this.postDAO.save(postDTO);
        ...
        if (files) {
            ...
            const fileUpload = await this.fileDAO.insertMany(fileDTOs);
    ...
    public deletePost: RequestHandler<HeaderId, Record<string, unknown>, Record<string, unknown>> = async (req, res) => {
        const { id } = req.params;
        ...
        await this.fileDAO.deleteMany({ postId: post.id });
        
        const postDeleted = await this.postDAO.delete(id);

        res.status(200).send({ success: true, message: "Post deleted successfully", data: postDeleted });
    }

    public updatePost: RequestHandler<HeaderId, Record<string, unknown>, UpdatePostBody> = async (req, res) => {
        const { id } = req.params;
        const { title, content, tags } = req.body;
        const author = req?.userId;
        ...
        const updatedPost = await this.postDAO.update(id, postDTO);
